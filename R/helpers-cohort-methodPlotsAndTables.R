...
# used in estimation-power
prepareCohortMethodPowerTable <- function(mainResults, connectionHandler , resultsSchema, tablePrefix) {
...
prepareCohortMethodSubgroupTable <- function(subgroupResults, output = "latex") {
  rnd <- function(x) {
...
drawCohortMethodAttritionDiagram <- function(attrition,
                                           targetLabel = "Target",
...
nonZeroCohortMethodHazardRatio <- function(hrLower, hrUpper, terms) {
  if (hrUpper < 1) {
    return(terms[1])
  } else if (hrLower > 1) {
    return(terms[2])
...
prettyCohortMethodHr <- function(x) {
  if (!is.numeric(x)) {
    x <- as.numeric(x)
  }
  result <- sprintf("%.2f", x)
...
# used in here
goodCohortMethodPropensityScore <- function(value) {
  return(value > 1)
}

# used in here
goodCohortMethodSystematicBias <- function(value) {
  return(value > 1)
}
...
plotCohortMethodForest <- function(results, limits = c(0.1, 10), metaAnalysisDbIds = NULL) {
